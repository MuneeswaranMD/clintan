    ...
    return (
        <div className="space-y-10 animate-fade-in pb-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {loading ? (
                    <div className="col-span-full py-40 text-center text-xl font-bold text-slate-300 uppercase tracking-widest animate-pulse italic">Synchronizing Automations...</div>
                    ...
                    <div key={item.id} className="bg-white p-8 rounded-xl border border-slate-200 hover:border-blue-400 hover:shadow-md transition-all flex flex-col group relative overflow-hidden">
                            ...
                            <div className="grid grid-cols-2 gap-4 border-t border-slate-50 pt-6 mb-8">
                                <div>
                                    ...
                                    <div className="flex items-center gap-1.5 font-bold text-slate-700 text-xs">
                                        <Calendar size={12} className="text-blue-600" /> {new Date(item.nextRun).toLocaleDateString()}
                                    </div>
                                </div>
                                <div className="text-right">
                                    <p className="text-[9px] font-bold text-slate-400 uppercase mb-1 tracking-wider">Value / Cycle</p>
                                    <p className="text-xl font-bold text-slate-900 tracking-tighter">₹{item.amount.toLocaleString()}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="flex gap-3 relative z-10">
                            ...
                            <button className="w-12 h-12 bg-white border border-slate-200 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-xl flex items-center justify-center transition-all">
                                <Settings size={18} />
                            </button>
                        </div>
                    </div>
                    ...
                    <div className="col-span-full py-40 bg-slate-50 rounded-2xl border-2 border-dashed border-slate-200 text-center">
                        ...
                        <p className="text-slate-500 text-sm max-w-xs mx-auto mb-8">Setup recurring invoices to automate your revenue streams and save time.</p>
